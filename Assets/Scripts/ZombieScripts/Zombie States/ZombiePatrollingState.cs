...
public class ZombiePatrollingState : ZombieBaseState
{
    ...
    public override BaseState<ZombieContext> UpdateState(ZombieContext context)
    {
        ...
        distance = Vector3.Distance(context.transform.position, context.currentTarget);
        if (distance < context.zombieNavMeshAgent.stoppingDistance) return context.idleState;
